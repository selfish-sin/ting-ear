...
- **本地向量知识库**（`local-ingest`/`vector-store`/`embedding-caller`）：
  ...
  - **坑（已修）**：nmem 远程索引的不确定进度条只在用户**主动**点「同步本书」时显示（`nmemManualSyncing` 标志）；自动后台 ingest（`autoIngestBook`/探针 `catchUp`）虽让 `bookIngestStatus` 变 submitting/indexing，但不显示进度条——否则用户没点向量化按钮也会看到假加载界面，无法辨识是否真正向量化过
