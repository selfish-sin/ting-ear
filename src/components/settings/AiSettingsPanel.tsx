...
import { ChevronDown, Eye, EyeOff, Plus, RefreshCw, Trash2, Zap } from 'lucide-react'
          ...
          return (
            ...
            >
              ...
              <input
                ...
                onChange={(e) => updateEngine(engine.id, { baseUrl: e.target.value })}
                className={inputClass}
                placeholder="API 地址，如 https://open.bigmodel.cn/api/paas/v4"
              />
              {engine.baseUrl && (
                <p className="truncate text-[11px] text-gray-400 dark:text-gray-500">
                  实际请求：{engine.baseUrl.replace(/\/+$/, '')}/chat/completions
                </p>
              )}
          ...
          <div className="mt-4 space-y-5">
            ...
            <div className="space-y-3">
              ...
              {([
                ...
                ['fullTextInjectPrompt', '本章注入提示词（会话内仅一次，按当前章）']
              ...
              <label className="block">
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    onChange({
                      ...value,
                      webSearch: { ...value.webSearch, enabled: value.webSearch?.enabled ?? false, prompt: e.target.value }
                    })
                  }
                  className={`${inputClass} resize-y leading-6`}
                />
